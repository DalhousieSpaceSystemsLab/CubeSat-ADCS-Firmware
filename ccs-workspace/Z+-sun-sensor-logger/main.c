...
{
    int8_t rslt;
    SUNSEN_measurement_t sunsen_readings;
    ...
    while (rslt == 0 )
    {
        rslt = SUNSEN_get_z_pos_temp();//IMU_get_gyro(&gyro_readings);
        sunsen_readings = SUNSEN_get_face_lux(SUNSEN_FACE_z_pos);
        //uart_printf("%d %d %d\n", sunsen_readings.lux_1, sunsen_readings.lux_2, sunsen_readings.lux_3);
